// Hidden Secret Key
const SECRET_KEY = "Elvare50";
// Specific dates the form is open
const OPEN_DATES = [5, 7, 9, 11, 13, 15, 17, 19, 20];

window.onload = function() {
    const savedName = localStorage.getItem('player_name');
    if (savedName) {
        showMainForm(savedName);
        renderProgress();
    }
    checkTime();
};

function saveName() {
    const nameInput = document.getElementById('user-name');
    const name = nameInput.value.trim();
    if (name) {
        localStorage.setItem('player_name', name);
        showMainForm(name);
    }
}

function showMainForm(name) {
    document.getElementById('name-group').style.display = 'none';
    document.getElementById('digit-group').style.display = 'block';
    document.getElementById('display-name').innerText = name;
}

function checkTime() {
    const now = new Date();
    const day = now.getDate();
    const status = document.getElementById('timer-status');
    const wrap = document.getElementById('input-wrap');
    
    // Check if today's date is in our allowed list
    if (OPEN_DATES.includes(day)) {
        status.innerText = "● STATUS: OPEN";
        status.style.color = "#00ff41";
        wrap.style.display = "block";
    } else {
        status.innerText = "● STATUS: CLOSED (Wait for next date)";
        status.style.color = "#ff4141";
        wrap.style.display = "none";
    }
}

function submitChar() {
    const input = document.getElementById('char-input');
    const val = input.value.trim();
    const name = localStorage.getItem('player_name');
    const now = new Date();
    const day = now.getDate();
    
    if (val.length === 1) {
        let progress = JSON.parse(localStorage.getItem('user_chars') || "[]");
        let lastSubmitDay = localStorage.getItem('last_submit_day');

        // Logic for 1 character per day, except on the 20th
        if (lastSubmitDay == day && day !== 20) {
            alert("You already submitted today! Come back on the next open date.");
            return;
        }

        if (progress.length < 8) {
            progress.push(val);
            localStorage.setItem('user_chars', JSON.stringify(progress));
            localStorage.setItem('last_submit_day', day);
            renderProgress();
            input.value = "";

            // If they reach 8, handle completion
            if (progress.length === 8) {
                handleCompletion(name, progress.join(''));
            } else if (day === 20) {
                alert("Character saved! Since it is the final day, you can enter another if you are missing any.");
            } else {
                alert("Character saved! Progress updated.");
            }
        }
    }
}

function handleCompletion(name, fullCode) {
    // Hidden internal check
    if (fullCode === SECRET_KEY) {
        logActivity(`${name} successfully completed the challenge!`);
    } else {
        logActivity(`${name} has submitted all 8 characters.`);
    }
    alert("Full code submitted! Check Instagram for results.");
}

function renderProgress() {
    const progress = JSON.parse(localStorage.getItem('user_chars') || "[]");
    const display = document.getElementById('progress-display');
    let str = "";
    for (let i = 0; i < 8; i++) {
        str += (progress[i] ? progress[i] : "_") + " ";
    }
    display.innerText = str;
}

function logActivity(msg) {
    const feed = document.getElementById('activity-feed');
    const entry = document.createElement('div');
    entry.style.marginBottom = "5px";
    entry.innerText = `> ${msg}`;
    feed.prepend(entry);
}
